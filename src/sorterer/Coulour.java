...
public class Coulour {
    ...
    private static int vent = 1000; // setter ventetid for hvorlenge sortereren
	...
	LCD.drawString("Klar...", 0, 0); // skriver ut at programmet er klart
					 // til å kjøre
	...
	ColorHTSensor colorSensor = new ColorHTSensor(SensorPort.S2); // forteller
								      // NXT I
								      // hvilken
	...
	while (fortsett) { // starter løkken for sortering og kalibrering
		...
		colorSensor.initBlackLevel(); // setter kalibrerer til
					      // svartnivåer
	    }
	    if (Button.LEFT.isDown()) {
		colorSensor.initWhiteBalance(); // setter kalibrerer til
						// hvitbalanse
	    }
		...
		LCD.drawString("R: " + rgb.getRed() + " G: " + rgb.getGreen() + " B: " + rgb.getBlue(), 0, 5); // skreiver
													       // RGB
													       // verdien
													       // på
													       // LCD
	    ...
	    if (btc.getDone() || Button.ESCAPE.isDown()) { // stopper løkken om
							   // escape knappen
							   // blir brukt
		fortsett = false;
